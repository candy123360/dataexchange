...
import cn.csservice.dataexchange.service.ExternalReceiveService;
...
public class ExternalReceiveController {
	private static Logger logger = Logger.getLogger(ExternalReceiveController.class);
	@Autowired
	ExternalReceiveService externalReceiveService;
	...
	public Object receiveJson(@RequestBody Sql sql) {
		...
		boolean bTrusted = externalReceiveService.isTrusted(clientIp);
		if (!bTrusted) {
		...
		boolean bReceivable = externalReceiveService.isReceivableTable(sql.getSql());
		if (!bReceivable) {
			obj.put("state", "failure");
		...
		externalReceiveService.processSql(sql.getSql());
		return obj.toString();
	}
}
